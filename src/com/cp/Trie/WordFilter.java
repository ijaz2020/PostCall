...
public class WordFilter {
    public static void main(String[] args) {
        String[] words = {"apple", "raj", "willow", "rjkhbbkla"};
        WordFilter wordFilter = new WordFilter(words);
        System.out.println(wordFilter.f("a", "e"));
        System.out.println(wordFilter.f("r", "j"));
        System.out.println(wordFilter.f("r", "a"));
        System.out.println(wordFilter.f("w", "w"));
    }
    ...
    public void findf(TrieNode node, String suffix, int len) {
        ...
        for (int i = 0; i < 26; i++) {
            if (node.children[i] != null)
                findf(node.children[i], suffix, len);
        }
    }
}
